...
public class LunarCalendarUtils {
    ...
    public String getLunarTerm() {
        if (!lunarTerm.isEmpty()) {
            return "二十四节气：" + lunarTerm;
        } else {
            return lunarTerm;
        }
    }
    ...
    public static CalendarInfo getCurrentDate(){
        ...
        LunarCalendarUtils festival = new LunarCalendarUtils();
        festival.initLunarCalendarInfo(formattedDate);
        CalendarInfo backInfo = new CalendarInfo();
        backInfo.setSolarDate(String.format("公历：%s", formattedDate));
        backInfo.setLunarDate(String.format("农历：%s【%s】年 %s月%s", festival.getGanZhiYear(), festival.getAnimal(), festival.getLunarMonth(), festival.getLunarDay()));
        backInfo.setLunarTerm(festival.getLunarTerm());
        backInfo.setLunarFestival(festival.getLunarFestival());
